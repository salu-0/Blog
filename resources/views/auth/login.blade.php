...
<body class="font-sans antialiased bg-black overflow-hidden relative h-screen w-screen">
    ...
    <div class="relative z-10 w-full h-full flex items-center justify-center p-4">
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-4">
                    ...
                    <div>
                        <a href="{{ route('password.request') }}"
                            class="text-sm font-semibold text-gray-900 hover:underline">Forgotten your password?</a>
                    </div>

                    <button
                        class="w-full bg-blue-600 hover:bg-blue-700 active:scale-[0.98] text-white font-bold py-3 rounded-full transition-all text-base shadow-sm">
                        Log in
                    </button>
        ...
        <div class="fixed bottom-0 w-full z-20 bg-black/80 backdrop-blur-sm text-center py-3">
            <a href="{{ route('register') }}" class="text-white font-bold text-sm tracking-wide hover:underline">Not on
                Freshwave yet? Sign up</a>
        </div>
    </div>
